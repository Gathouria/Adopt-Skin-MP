...
namespace AdoptSkinMpBeta.Framework
{
    class HoverBox
    {
        ...
        internal void HoverCheck()
        {
            ...
            // Show pet tooltip
            if (ModApi.GetPets() != null &&
                ModApi.GetPets().Count() > 0)
            ...
            // Show horse tooltip
            if (ModApi.GetHorses() != null &&
                ModApi.GetHorses().Count() > 0)
                foreach (Horse horse in ModApi.GetHorses())
                    if (IsWithinSpriteBox(mousePos, horse) && horse != Game1.player.mount)
                    {
                        isHovering = true;
